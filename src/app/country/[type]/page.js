...
export default function CountryPage({ params }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const q = query(collection(db, "test"), where("culture", "==", type));
  ...
  return (
    ...
    >
      ...
      <div className="pt-20 relative transition-all duration-300 ease-in-out">
        ...
        >
          <h1 className="text-3xl font-bold mb-6">🌏 {type} Cuisine</h1>
          ...
          {meals.length > 0 ? (
            ...
            <p className="text-gray-500">No meals found for this cuisine.</p>
          )}
        </main>
      </div>
    </div>
